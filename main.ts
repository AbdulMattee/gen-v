...
async function findEnvVars(directory: string, excludeFolders: string[], envPatterns: RegExp[]): Promise<Set<string>> {
...
function getOutputFilePath(outputDirectory: string, outputFileName: string, overWritePrevEnv: boolean): string {
...
async function main(): Promise<void> {
  ...
  try {
    const { print, directory, exclude, name, output, overwrite, prefixes } = argv;
    ...
    const envVars = await findEnvVars(directory, excludedDirectories, prefixesRegExp);
    const sortedVars = Array.from(envVars).sort();

    if (envVars.size > 0) {
      console.log(BLUE_COLOR, 'Total environment variables found:', GREEN_COLOR, envVars.size);
    }
    ...
    console.log(
      ...
      'Excluded folders:',
      GREEN_COLOR,
      JSON.stringify(excludedDirectories.filter((folder: string) => folder !== directory)),
    );
